...
from rclip.const import IS_LINUX, IS_MACOS
...
def _import_onnxruntime():
    ...
    with tempfile.TemporaryFile(mode="w+b") as stderr_capture:
        ...
        filtered_stderr = _filter_onnxruntime_stderr(
            stderr_capture.read().decode(errors="replace")
        )
...
def ensure_downloaded() -> None:
    ...
    if not os.path.isfile(visual_query_path):
        to_download.append(
            (
                f"{MODEL_SUBDIR}/{VISUAL_ONNX}",
                lambda tqdm_class: download_visual_query_model(tqdm_class=tqdm_class),
            )
        )
    ...
    if not os.path.isfile(textual_path):
        to_download.append(
            (
                f"{MODEL_SUBDIR}/{TEXTUAL_ONNX}",
                lambda tqdm_class: download_textual_model(tqdm_class=tqdm_class),
            )
        )
        ...
        if os.path.isdir(visual_index_path):
            ...
            to_download.append(
                (
                    ...
                    lambda tqdm_class: download_visual_index_model_package(
    ...
    if not os.path.isfile(tokenizer_path):
        to_download.append(
            (
                TOKENIZER_VOCAB,
                lambda tqdm_class: download_tokenizer_vocab(tqdm_class=tqdm_class),
            )
        )
    ...
    size_by_file = {
        repo_file.rfilename: repo_file.size or 0
        for repo_file in (repo_info.siblings or [])
    }
    selected_prefixes = [prefix for prefix, _download_function in to_download]
    total_bytes = sum(
        size
        for path, size in size_by_file.items()
        if any(path.startswith(prefix) for prefix in selected_prefixes)
    )

    shared_bar = tqdm_cls(
        total=total_bytes, desc="Downloading model", unit="B", unit_scale=True
    )
